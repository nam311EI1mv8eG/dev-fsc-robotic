<div class="content">
    @can('match_team_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.match-teams.create') }}">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-matchMatchTeams">
                            <thead>
                                <tr>
                                    <th width="10">
                            ...
                            <tbody>
                                @foreach($matchTeams as $key => $matchTeam)
                                    <tr data-entry-id="{{ $matchTeam->id }}">
                                        ...
                                        <td>
                                            @can('match_team_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.match-teams.show', $matchTeam->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('match_team_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.match-teams.edit', $matchTeam->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('match_team_delete')
                                                <form action="{{ route('admin.match-teams.destroy', $matchTeam->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('match_team_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.match-teams.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-matchMatchTeams:not(.ajaxTable)').DataTable({ buttons: dtButtons })
