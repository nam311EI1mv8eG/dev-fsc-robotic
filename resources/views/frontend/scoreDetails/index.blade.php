        ...
        <div class="col-md-12">
            @can('score_detail_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.score-details.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $scoreDetail->id }}">
                                        ...
                                        <td>
                                            @can('score_detail_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.score-details.show', $scoreDetail->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('score_detail_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.score-details.edit', $scoreDetail->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('score_detail_delete')
                                                <form action="{{ route('frontend.score-details.destroy', $scoreDetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('score_detail_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.score-details.massDestroy') }}",
