        ...
        <div class="col-md-12">
            @can('season_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.seasons.create') }}">
                            ...
                            <tbody>
                                @foreach($seasons as $key => $season)
                                    <tr data-entry-id="{{ $season->id }}">
                                        ...
                                        <td>
                                            @can('season_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.seasons.show', $season->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('season_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.seasons.edit', $season->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('season_delete')
                                                <form action="{{ route('frontend.seasons.destroy', $season->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('season_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.seasons.massDestroy') }}",
